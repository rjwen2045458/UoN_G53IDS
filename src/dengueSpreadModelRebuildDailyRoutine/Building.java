...
//		DengueSpreadModel model = (DengueSpreadModel) state;
		
		DengueSpreadModelRebuildDailyRountine model = (DengueSpreadModelRebuildDailyRountine) state;
	...
	public void InfectedDefault(DengueSpreadModelRebuildDailyRountine model) {
	...
	public void InfectedLinear(DengueSpreadModelRebuildDailyRountine model) {
		// whether the building is infected
		Bag PeopleVisited = model.socialNetwork.getEdgesIn(this);
		boolean infected = false;
		for (int i = 0; i < PeopleVisited.size(); i++) {
	...
	public void InfectedPolynominal(DengueSpreadModelRebuildDailyRountine model) {
		// whether the building is infected
		Bag PeopleVisited = model.socialNetwork.getEdgesIn(this);
		boolean infected = false;
		for (int i = 0; i < PeopleVisited.size(); i++) {
