...
public class DengueSpreadModelConstantDailyRountine extends SimState implements ModelConfig{
	...
	int initialPatient = 100;
	
	public DengueSpreadModelConstantDailyRountine(long seed) {
		super(seed);
	}
	...
	public void start() {
		...
		for (int i = 0; i < People; i++) 
			...
			people.add(new Person((int) Distributions.nextWeibull(Shape, Scale, random)+2));
			
		// assign people to their home
		Bag peopleToHome  = new Bag(people);
		Poisson PD = new Poisson(3, random); // poission distribution
		...
		for (int i = 0; i < People; i++) {
			...
			int maxNumBuildings = P.getMaxPlaces();
			...
			for (int j = 0; j < maxNumBuildings-1; j++)
				places.add((Building) buildingToChoose.remove(random.nextInt(buildingToChoose.size())));

			// assign time slots
			int totalTimeLeft = 16;
			Exponential e = new Exponential(1,random);
			int counter = 0;
			int[] timeSlots = new int[maxNumBuildings]; 
			for(int j = 0; j < maxNumBuildings; j++)
				timeSlots[j] = 0;
			...
			while(totalTimeLeft > 0) {
				...
				if(totalTimeLeft < time) {
					...
					totalTimeLeft -= time;
					counter++;
					counter = counter % maxNumBuildings;
				}
			}


			// Clean the places assigned 0 hour
			for(int j = maxNumBuildings-1; j > -1; j--) {
				if(timeSlots[j] == 0) {
					places.pop();
				}
			}
			...
			P.setRoutinePlaces(numPlaces);
			
			// build social network edges
			for(int j = 0; j <numPlaces; j++) {
				socialNetwork.addEdge(new Edge(P, places.get(j), timeSlots[j]));
			}
		}
...
//		System.out.println(b);
	...
	public static void main(String[] args) {
		...
		SimState state = new DengueSpreadModelConstantDailyRountine(System.currentTimeMillis()); // MyModel is our SimState subclass
		System.out.println(InfectiousCoefficientLinear);
		System.out.println(((DengueSpreadModelConstantDailyRountine)state).initialPatient);
