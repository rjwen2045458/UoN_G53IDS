...
public class DengueSpreadModelwithUI extends GUIState
{
	...
	public DengueSpreadModelwithUI() { super(new DengueSpreadModelConstantDailyRountine( System.currentTimeMillis())); }
	...
	public void updateData() {
		ChartUtilities.scheduleSeries(this, exposedPeople, new sim.util.Valuable(){
			public double doubleValue(){
				return ((DengueSpreadModelConstantDailyRountine)state).Exposed_People;
			}
		});
		ChartUtilities.scheduleSeries(this, infectiousPeople, new sim.util.Valuable(){
			public double doubleValue(){
				return ((DengueSpreadModelConstantDailyRountine)state).Infectious_People;
			}
		});
		ChartUtilities.scheduleSeries(this, revivingPeople, new sim.util.Valuable(){
			public double doubleValue(){
				return ((DengueSpreadModelConstantDailyRountine)state).Reviving_People;
			}
		});
		ChartUtilities.scheduleSeries(this, resistedPeople, new sim.util.Valuable(){
			public double doubleValue(){
				return ((DengueSpreadModelConstantDailyRountine)state).Resisted_People;
			}
		});
		ChartUtilities.scheduleSeries(this, infectiousPlaces, new sim.util.Valuable(){
			public double doubleValue(){
				return ((DengueSpreadModelConstantDailyRountine)state).Infectious_Places;
			}
		});
	}
